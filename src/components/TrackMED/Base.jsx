...
export class Base extends Component {
    /* Notice that <th># Components</th> and <td>{ description.noOfComponents }</td> are conditionally-rendered
       See https://stackoverflow.com/questions/59819047/conditional-form-rendering-in-react
    */
        ...
        return <table className='table-striped table-condensed table-hover' cellSpacing='0' width='100%'>
            ...
            {descriptions.map( (description, index) =>
                <tr key={ index + 1 }>
                    ...
                    { this.state.descTable === 'Owner' && (
                        <td>{ description.noOfComponents }</td>)
                    }                    
    ...
    */
    ...
    // regular (ie., not mui-datatable) table for child components
    ...
    // regular (ie., not mui-datatable) table for child systems
    ...
    // mui-datatable for child systems and components table
    popupComponentDT(root) {
        ReactDOM.render( 
        <td colSpan = '7'> 
            <AsyncFetchPop data = { this.state.dtComponents } title = { this.state.dtTitle } />
        </td>, root);
    }
    ...
    getItems(itemApi) {
      ...
      var actionRoute = this.title === "Owner"? "/GetNoOfComps": "";
      
      // fetch(this.itemUrl + itemApi)
      fetch(this.itemUrl + itemApi + actionRoute)
        .then(response => response.json())
        .then(data => {
            console.log(data);
            /* Sample output
            2: {…}
            createdAtUtc: "2017-09-17T00:14:17.732Z"
            desc: "Freddie Miranda"
            id: "59bdbe5c1bd00316e834ae2d"
            noOfComponents: 35
            <prototype>: {…
             */
            this.setState({ descriptions: data, loading: false, descTable: this.title});
      });
    }
   
    // used only for dynamic nested plain HTML components tables: invoked from createNTableHTML method
    headings = {
        'assetnumber' : 'Asset#',
        'imte' : 'IMTE',
    ...
    // used only for dynamic nested plai HTML child systems tables: invoked from createNTableHTML method
    headingsSystem = {
        'imte' : 'IMTE',
    ...
    /* OPTIONS for CREATING NESTED TABLES */

    // option 1: mui-datatables
    createDataTable(id, elGP, elP){
        ...
        .then(data => {
            ...
            if( this.title === 'Description' || 
                ...
                this.setState({ dtComponents: data, dtTitle: "Component", loading: false });
                this.popupComponentDT(this.tr); 
        
            } else if( this.title === 'Location') {
                this.setState({ dtComponents: data, dtTitle: "System", loading: false });
                this.popupComponentDT(this.tr); 
            }  
    ...
    // option 2: dynamically compose nested table: Using plain HTML elements (needs a revision for Location tables)
    ...
    // option 3: dynamically compose nested table: using pop-up react components
